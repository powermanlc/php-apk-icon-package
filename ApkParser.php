...
//----------------------
  ...
  public function getMinSdkVersion(){
    return $this->getAttribute('manifest', 'android:minSdkVersion');
  }
  public function getChannel(){
    return $this->getAttribute('manifest/application/meta-data', 'android:UMENG_CHANNEL');
  }
  public function getIcon(){
    return $this->getAttribute('manifest/application', 'android:icon');
  }
...
#var_dump($r);die;
//    var_dump(array_keys($r));die;
//    var_dump($r['child'][0]);die;
    ...
    if (empty($return) && isset($r['child'])) {
        foreach($r['child'] as $ac) {
            if (isset($ac['attrs'])) { 
                foreach($ac['attrs'] as $aa) {
                    if ($aa['ns_name'] == $name) {
                        $return = $this->getAttributeValue($aa);
                        break;
                    }
                }
            }
  ...
  private function parseBlock($need = 0){
    ...
    switch ($type){
      case self::AXML_FILE:
        $props = array(
          'line' => 0,
          'tag' => '<?xml version="1.0" encoding="gb2312"?>'
        );
      break;
